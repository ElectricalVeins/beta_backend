...
import { DataSource, EntityManager, FindManyOptions, FindOptionsRelations } from 'typeorm';
...
import { S3ObjectTypesEnum, S3Service } from '../../external/s3/s3.service';
...
export class LotService implements OnApplicationBootstrap {
  private logger = new Logger(LotService.name);
  ...
  startLotTimer(payload: LotTimerPayload): any {
    const timeoutId = setTimeout(() => this.closeLot(payload.id), payload.milliseconds);
    this.logger.log(`Lot.Close:${payload.id} - ${timeoutId}`);
  }
